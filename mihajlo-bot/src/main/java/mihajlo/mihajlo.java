...
public class mayari extends AIWithComputationBudget {
    ...
    //changed this function to get hopefully better results?? *test it against Mayari
    public UnitAction calculatePath(Unit unit, Pos destination, int maximumDistance) {
    final int width = _pgs.getWidth();
    final int height = _pgs.getHeight();
    int[][] distances = new int[width][height];
    for (int[] distanceRow : distances) {
        Arrays.fill(distanceRow, Integer.MAX_VALUE);
    }
    distances[destination.getX()][destination.getY()] = 0;

    Queue<Pos> toExplore = new LinkedList<>();
    toExplore.add(destination);
    int currentDistance = 1;

    while (!toExplore.isEmpty() && currentDistance <= maximumDistance) {
        Queue<Pos> nextToExplore = new LinkedList<>();
        while (!toExplore.isEmpty()) {
            Pos current = toExplore.poll();
            List<Pos> neighbours = allPosDist(current, 1);
            for (Pos neighbour : neighbours) {
                boolean isNeighbourBlocked = isBlocked(unit, neighbour);
                boolean isNeighbourVisited = distances[neighbour.getX()][neighbour.getY()] != Integer.MAX_VALUE;
                if (!isNeighbourBlocked && !isNeighbourVisited && !nextToExplore.contains(neighbour)) {
                    distances[neighbour.getX()][neighbour.getY()] = currentDistance;
                    nextToExplore.add(neighbour);
                }
            }
        }
        if (distances[unit.getX()][unit.getY()] != Integer.MAX_VALUE) {
            break;
        }
        currentDistance += 3; // skipping two steps as in the original algorithm
        toExplore = nextToExplore;
    }

    // determining the best move based on the calculated distances
    Pos unitPos = toPos(unit);
    return allPosDist(unitPos, 1).stream()
            .filter(p -> !outOfBound(p) && _pgs.getTerrain(p.getX(), p.getY()) != TERRAIN_WALL && distances[p.getX()][p.getY()] != Integer.MAX_VALUE && _pgs.getUnitAt(p.getX(), p.getY()) == null)
            .map(p -> new AbstractMap.SimpleEntry<>(p, -1000 * distances[p.getX()][p.getY()] - (int) squareDist(p, destination)))
            .max(Map.Entry.comparingByValue())
            .map(bestMove -> new UnitAction(UnitAction.TYPE_MOVE, toDir(unitPos, bestMove.getKey())))
            .orElse(null);
}
    ...
    //this one should be more efficient now
    public UnitAction findAdjacentPath(Unit source, Integer destinationIndex) {
    int destinationX = destinationIndex % _pgs.getWidth();
    int destinationY = destinationIndex / _pgs.getWidth();
    Pos destinationPos = new Pos(destinationX, destinationY);
    
    UnitAction pathThroughAstar = _astarPath.findPathToAdjacentPosition(source, destinationIndex, _gs, fullResourceUse());
    if (pathThroughAstar == null || timeRemaining(false) <= 35) {
        return pathThroughAstar;
    }
    
    int searchRadius = _pgs.getUnits().size() > 32 ? 42 : 64;
    UnitAction alternativePath = findPath(source, destinationPos, searchRadius);
    
    // return the alternative path if found; otherwise, return the A* path
    return alternativePath != null ? alternativePath : pathThroughAstar;
}
    ...
    public Pos futurePos(int currentX, int currentY, int direction) {
    int maxX = _pgs.getWidth() - 1;
    int maxY = _pgs.getHeight() - 1;

    int updatedX = currentX + ((direction == UnitAction.DIRECTION_RIGHT && currentX < maxX) ? 1 : (direction == UnitAction.DIRECTION_LEFT && currentX > 0) ? -1 : 0);
    int updatedY = currentY + ((direction == UnitAction.DIRECTION_DOWN && currentY < maxY) ? 1 : (direction == UnitAction.DIRECTION_UP && currentY > 0) ? -1 : 0);

    return new Pos(updatedX, updatedY);
}
    ...
    void basesAction() {
        ...
        int targetWorkers = calculateTargetWorkerCount();
        
        _bases.forEach(base -> {
            if (isBaseBusy(base) || !canProduceMoreWorkers(activeProductionCount)) {
                return;
            }
            
            if (isProductionCritical()) {
                initiateWorkerProduction(base, workerType, true);
                activeProductionCount++;
                return;
            }
            
            boolean productionSuccess = tryDirectWorkerProduction(base, workerType);
            if (!productionSuccess) {
                productionSuccess = initiateWorkerProduction(base, workerType, false);
            }
    ...
    boolean isBaseBusy(Unit base) {
        return _gs.getActionAssignment(base) != null;
    }

    int calculateTargetWorkerCount() {
        int workerPerBase = _bases.isEmpty() ? 0 : workerPerBase(_bases.get(0));
        return workerPerBase * _bases.size();
    }

    boolean canProduceMoreWorkers(long currentProductionCount) {
        return _pgs.getWidth() < 9 || _workers.size() + currentProductionCount < calculateTargetWorkerCount();
    }

    boolean isProductionCritical() {
        // This can be expanded to check if workers carrying resources might tip the balance
        return _resources.isEmpty() && ((_p.getResources() - _resourcesUsed) == 1);
    }

    boolean tryDirectWorkerProduction(Unit base, UnitType workerType) {
        int directionForProduction = bestBuildWorkerDir(base);
        return produce(base, directionForProduction, workerType);
    }

    boolean initiateWorkerProduction(Unit base, UnitType workerType, boolean anywhere) {
        if (anywhere) {
            return produceWherever(base, workerType);
        } else {
            return tryDirectWorkerProduction(base, workerType);
        }
    }
